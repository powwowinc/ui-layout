...
angular.module('ui.layout', [])
    ...
    function uiLayoutCtrl($scope, $attrs, $element, $timeout, $window, LayoutContainer, Layout) {
      ...
      ctrl.mouseMoveHandler = function (mouseEvent) {
        var mousePos = mouseEvent[ctrl.sizeProperties.mouseProperty] ||
          ...
          ($window.jQuery ? (mouseEvent.originalEvent
            ? mouseEvent.originalEvent.targetTouches[0][ctrl.sizeProperties.mouseProperty]
            : 0) : (mouseEvent.targetTouches ? mouseEvent.targetTouches[0][ctrl.sizeProperties.mouseProperty] : 0));
      ...
      ctrl.isPercent = function (num) {
        return !!(num && angular.isString(num) && num.indexOf('%') > -1);
      };
      ...
      ctrl.calculate = function () {
        ...
        if (ctrl.containers.length > 0 && $element.children().length > 0) {
          ...
          // FIXME: autoSize if frequently Infinity, since numOfAutoContainers is frequently 0, no need to calculate
          // that set the sizing for the ctrl.containers
          /*
          ...
          for (i = 0; i < ctrl.containers.length; i++) {
            ...
            if (LayoutContainer.isSplitbar(c)) {
              ...
              c.size = (newSize === null) ? autoSize : newSize;
            }
      ...
      ctrl.toggleBefore = function (splitbar) {
        ...
        // FIXME: collapse:resize:uncollapse: works well "visually" without the nextSplitbar and nextContainer
        // calculations but removing those breaks few test
      ...
      ctrl.toggleAfter = function (splitbar) {
        ...
        // FIXME: collapse:resize:uncollapse: works well "visually" without the prevSplitbar and prevContainer
        // calculations but removing those breaks few test
        $scope.$apply(function () {
          if (c.collapsed) {
            ...
            return {
              ...
              post: function (scope, element, attrs, ctrl) {
                ...
                var splitbar = angular.element('<div ui-splitbar>' +
                  ...
                  '<a><span class="ui-splitbar-icon"></span></a>' +
                  '</div>');
                if (index > 0 && !ctrl.hasSplitbarBefore(scope.container)) {
  ...
  .directive('uiLayoutLoaded', ['$timeout', 'Layout', function ($timeout, Layout) {
    ...
    // because prog. toggling depends on the logic of prevButton and nextButton (which should be probably refactored
    // out)  This is how it currently works: 1. uiLayoutContainer in prelink phase resets @collapsed to false, because
    // layout has to be calculated with all containers uncollapsed to get the correct dimensions 2. layout with
    // ui-layout-loaded attributes broadcasts "ui.layout.loaded" 3. user changes values of @collapsed which triggers
    // 'click' on either of the buttons 3. the other button is hidden and container size set to 0
    return {
      require: '^uiLayout',
      restrict: 'A',
      priority: -100,
